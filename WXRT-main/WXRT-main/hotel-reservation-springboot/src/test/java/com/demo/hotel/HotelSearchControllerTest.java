...
public class HotelSearchControllerTest {
    ...
    public void getHotels_HotelsFound_AddsResultsToModel() throws Exception {
        Hotel hotel = new Hotel("Hotel Royal", new Address("W Hotel", "10 Zhongxiao East Road Sec. 5, Xinyi Districtt", null,
        		City.TaipeiCity),
                5, "WTaipei.com");
        ...
        PageImpl<Hotel> results = new PageImpl<>(hotels, PageRequest.of(0, 20), hotels.size());
        when(hotelRepository.findAllByLocation(eq("TaipeiCity"), isNull(), isNull(), any(Pageable.class)))
                .thenReturn(results);
        ...
        mockMvc.perform(get("/hotel/search?citye=TaipeiCity"))
                .andExpect(status().isOk())
                .andExpect(view().name("/hotel/hotels"))
                .andExpect(model().attribute("hotels", hasExpectedPageResult));

        verify(hotelRepository, times(1))
                .findAllByLocation(eq("TaipeiCity"), isNull(), isNull(), any(Pageable.class));
    }
    ...
    public void getAvailableHotelRooms_HotelHasAvailableRooms() throws Exception {
        ...
        Address address = new Address("台北晶華酒店", "No. 3, Ln. 39, Sec. 2, Zhongshan N. Rd., Zhongshan Dist.", "",
