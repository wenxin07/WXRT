...
/**
 * Stores the {@code Reservation} and the current flow {@code Step}.
 * {@code Step} manipulation functions are dumb and rely on controller logic to
 * keep the flow step in sync.
 *
 * <p>
 * There cannot be an incrementing counter given page refreshes must not advance
 * flow steps.
 * </p>
 */
...
//   	        return Extras;
	...
	public ReservationFlow() {
		stepDescriptions.add(new StepDescription(0, "Dates", "選擇預定日期")); // Choose your reservation dates
		stepDescriptions.add(new StepDescription(1, "Guests", "房客資訊")); // Provide guest details
//      stepDescriptions.add(new StepDescription(2, "Extras", "Select optional extras"));

		stepDescriptions.add(new StepDescription(2, "Review", "確認訂房資訊")); // Verify your reservation
		stepDescriptions.add(new StepDescription(3, "Payment", "提供付款資訊")); // Provide payment details
	}
	...
	public static class StepDescription {
		...
		public String toString() {
			return "StepDescription{" + "flowStep=" + flowStep + ", title='" + title + '\'' + ", description='"
					+ description + '\'' + '}';
		}
	}
