...
public class ReservationDates {
    ...
    public Optional<ValidationError> validate(LocalDate now) {
        if (checkInDate == null) {
            return Optional.of(new ValidationError("checkInDate.missing", "入住時間請詳細填寫"));
        } else if (checkOutDate == null) {
            return Optional.of(new ValidationError("checkOutDate.missing", "退房時間請詳細填寫"));
        } else if (checkInDate.isBefore(now)) {
            return Optional.of(new ValidationError("checkInDate.future", "入住時間需大於目前日期"));
        } else if (checkOutDate.isBefore(checkInDate)) {
            return Optional.of(new ValidationError("checkOutDate.afterCheckIn", "退房時間不可早於入住間"));
        } else if (totalNights() < 1) {
            // handles case where check in/out dates are the same.
            return Optional.of(new ValidationError("checkOutDate.minNights", "住宿時長至少須一日"));
        } else if (policyAcknowledged == false) {
            return Optional.of(new ValidationError("policyAcknowledged.missing","請同意預留政策")); 
        }
